...
def generate_random_content() -> tuple[str, list[str]]:
    sentiment_type = random.choices(
        ["pos", "neg", "neu", "meta"], weights=[40, 30, 20, 10]
    )[0]
...
def run_memory_test():
    ...
    payload = {"messages": messages, "time_window_minutes": 60}
    ...
    print(
        f"RESUMO MEMÓRIA | Pico: {peak_mb:.2f} MB | Tempo: {round((t1 - t0) * 1000, 2)} ms | Status: {'APROVADO' if peak_mb < 20 else 'REPROVADO'}"
    )
